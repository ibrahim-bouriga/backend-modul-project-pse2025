...
export default function PriceCalculator({ configuration }: PriceCalculatorProps) {
  const { basePrice, color, wheels, interior, extras, totalPrice } = configuration;
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-lg border border-gray-200 divide-y divide-gray-200">
        ...
        {wheels.price > 0 && (
          <div className="p-4 flex justify-between items-center">
            <div>
              <p className="text-sm font-medium text-gray-700">{wheels.name}</p>
              <p className="text-xs text-gray-500">Wheel Style</p>
            </div>
            <p className="text-sm font-semibold text-blue-600">
              +${wheels.price.toLocaleString()}
            </p>
          </div>
        )}
        ...
        {interior.price > 0 && (
          <div className="p-4 flex justify-between items-center">
            <div>
              <p className="text-sm font-medium text-gray-700">{interior.name}</p>
              <p className="text-xs text-gray-500">Interior Package</p>
            </div>
            <p className="text-sm font-semibold text-blue-600">
