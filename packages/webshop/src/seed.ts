...
export async function seed() {
    ...
    for (const v of [
        ...
        await prisma.productVariant.upsert({ where: { sku: v.sku }, update: {}, create: { productId: tshirt.id, ...v } });
    }
    for (const v of [
        ...
        await prisma.productVariant.upsert({ where: { sku: v.sku }, update: {}, create: { productId: hoodie.id, ...v } });
    }
    for (const { sku, productId } of [
        ...
        await prisma.productVariant.upsert({ where: { sku }, update: {}, create: { productId, sku, options: {}, priceDelta: 0, stock: 20 } });
    }

    console.log('[Webshop] Seeding complete.');
}
